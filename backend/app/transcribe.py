...
class WhisperTranscriptionService:
    ...
    async def transcribe(
        ...
        try:
            ...
            # Optimized transcription parameters for maximum accuracy
            # Note: GPU models may have issues with certain parameters, so using safer defaults
            def transcribe_fn():
                # Use simpler parameters for GPU to avoid compatibility issues
                transcribe_params = {
                    "verbose": False,
                    "temperature": 0,  # Deterministic results
                    "best_of": 1,  # Set to 1 to avoid tensor size errors
                    "beam_size": 1,  # Use beam_size=1 to avoid tensor size errors
                ...
                # condition_on_previous_text can cause GPU issues, so only use on CPU
                if self._device == "cpu":
                    transcribe_params["condition_on_previous_text"] = True
                
                # initial_prompt can help with accuracy
                if language:
                    transcribe_params["initial_prompt"] = f"This is a {language} audio recording."
            ...
            # Debug: log the result structure to understand what Whisper returns
            logger.debug("Whisper result keys", keys=list(result.keys()) if isinstance(result, dict) else "not a dict")
            if isinstance(result, dict) and "text" in result:
                logger.debug("Raw transcript text", text_length=len(str(result.get("text", ""))))
            
            transcript = result.get("text", "").strip()
            detected_language = result.get("language", language or "unknown")
            segments = result.get("segments", [])
            
            # If transcript is empty but we have segments, try to reconstruct from segments
            if not transcript and segments:
                logger.warning("Transcript is empty but segments exist, reconstructing from segments", segment_count=len(segments))
                transcript = " ".join([seg.get("text", "").strip() for seg in segments if seg.get("text")]).strip()
            
            # Log if transcript is still empty
            if not transcript:
                logger.error(
                    "Transcription completed but transcript is empty",
